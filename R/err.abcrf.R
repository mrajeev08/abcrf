err.abcrf <- function(object, training, paral = FALSE, 
                      ncores = 1, 
                      plot = FALSE) {
  ...
  if (length(object$group) != 0) {
    ...
    for (k in 1:ngroup) {
      for (l in 1:length(object$group[[k]])) {
  ...
  inbag <- matrix(unlist(object$model.rf$inbag.counts, use.names = FALSE), 
                  ncol = object$model.rf$num.trees, byrow = FALSE)
  ...
  res <- oobErrors(
    sequo = as.integer(floor(sequo)), ntrain = as.integer(ntrain), 
    mod = as.integer(labels(modindex)),
    ntree = object$model.rf$forest$num.trees, modindex = as.numeric(modindex), 
    inbag = inbag, mimi = mimi
  )
  if(plot) {
    plot(floor(sequo), res, ylab = "Prior error rate", xlab = "Number of trees", 
         type = "l", ylim = range(res))
  }
