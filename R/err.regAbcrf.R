err.regAbcrf <- function(object, training, paral = FALSE,
  ...
  pred <- predict(object$model.rf, training, predict.all = TRUE, 
  ...
  if (what == "mean") {
    res <- oobErrorsReg(as.integer(floor(sequo)), as.integer(ntrain), 
                        as.integer(ntree), as.numeric(resp), inbag, pred)
  } else if (what == "median") {
    res <- oobMedErrorsReg(as.integer(floor(sequo)), as.integer(ntrain), 
                           as.integer(ntree), as.numeric(resp), inbag, pred)
  }

  if (plot) {
    plot(floor(sequo), res,
      ylab = "out-of-bag mean squared error",
      xlab = "Number of trees", type = "l"
    )
  }
