abcrf.formula <- function(formula, data, group = list(), lda = TRUE, ntree = 500,
  ...
  model.rf <- ranger(formula, data,
    num.trees = ntree, sample.fraction = sampsize / nrow(data),
  ...
  x <- list(call = cl, lda = lda, formula = formula, group = group, 
            model.rf = model.rf, model.lda = model.lda, 
            prior.err = model.rf$prediction.error)
  class(x) <- "abcrf"
  x
}
